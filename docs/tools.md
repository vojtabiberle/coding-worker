...
Model operations use the selected profile's engine (`opencode` or pinned Pi 0.78.1). Continuation and same-session report correction keep the saved profile and backend. `worker_verify`, `worker_wait`, result budgets, freshness and evidence validation are independent of the model backend. See [Pi configuration and limits](setup.md#pi-backend). Backend failure or missing capability never triggers a silent fallback. Pi's report and command events are evidence, not independent test acceptance; command exit status stays unknown when the protocol does not supply it.

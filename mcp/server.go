...
func New(a *worker.App) *sdk.Server {
	...
	sdk.AddTool(s, &sdk.Tool{Name: "worker_continue", Description: "Returns run_id/running acknowledgement; final payload is retrieved with worker_result after worker_wait returns done=true. Resume an implementation run's OpenCode session with feedback; returns updated implementation result. Rejects repository drift and non-implementation runs."}, func(ctx context.Context, _ *sdk.CallToolRequest, in worker.ContinueRequest) (*sdk.CallToolResult, any, error) {
